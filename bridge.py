...
BRIDGE_HOST = os.getenv("MATRIXFLOW_BRIDGE_HOST", "127.0.0.1")
BRIDGE_PORT = int(os.getenv("MATRIXFLOW_BRIDGE_PORT", "8000"))
BRIDGE_TOKEN = os.getenv("MATRIXFLOW_BRIDGE_TOKEN", "").strip()
BRIDGE_CORS_ORIGINS = [
    origin.strip()
    for origin in os.getenv(
        "MATRIXFLOW_BRIDGE_CORS_ORIGINS",
...
async def require_bridge_token(x_bridge_token: str = Header(default="")):
...
@app.post("/login/start", dependencies=[Depends(require_bridge_token)])
...
@app.get("/login/status/{session_id}", dependencies=[Depends(require_bridge_token)])
...
@app.post("/accounts/check", dependencies=[Depends(require_bridge_token)])
...
@app.post("/upload/video", dependencies=[Depends(require_bridge_token)])
...
@app.post("/upload/note", dependencies=[Depends(require_bridge_token)])
