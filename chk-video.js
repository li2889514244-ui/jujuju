...
(async()=>{
  const s = Buffer.from('grep -i "wechat\\|tencent\\|视频号\\|WECHAT_VIDEO\\|report-metrics" /root/.pm2/logs/matrixflow-out.log 2>/dev/null | tail -20; echo "---ERR---"; grep -i "wechat\\|tencent\\|视频号\\|WECHAT_VIDEO\\|report-metrics" /root/.pm2/logs/matrixflow-error.log 2>/dev/null | tail -10').toString('base64');
  const r1 = await c.request('CreateCommand',{RegionId: process.env.ALI_REGION || 'cn-guangzhou',Name:'chkv'+Date.now(),Type:'RunShellScript',CommandContent:s,ContentEncoding:'Base64',Timeout:'60'});
