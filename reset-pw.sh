...
HASH=$(node -e "const bcrypt=require('bcryptjs');console.log(bcrypt.hashSync('DY5p7eknNe0pdFiW',10))" 2>/dev/null)
if [ -z "$HASH" ]; then
  cd /opt/matrixflow && npm install bcryptjs --no-save 2>/dev/null
  cd /opt/matrixflow/backend
  HASH=$(node -e "const bcrypt=require('bcryptjs');console.log(bcrypt.hashSync('DY5p7eknNe0pdFiW',10))")
fi
echo "Hash: ${HASH:0:30}..."
