...
def _jitter_delay(base_seconds: float, spread_seconds: float = 1.0) -> float:
    """Return a small randomized delay so retries and pagination are not bursty."""
    return max(0.5, float(base_seconds or 0)) + random.uniform(0.0, max(0.0, spread_seconds))


def _first_url_from_avatar_fields(source: Dict[str, Any]) -> str:
    """Return the first usable avatar URL from Douyin user/author payloads."""
    ...
    for key in (
        ...
        url = first_url(source.get(key))
        if url:
            return url
    for key, value in source.items():
        lower = str(key or "").lower()
        if "avatar" in lower or "head" in lower:
            url = first_url(value)
            if url:
                return url
    return ""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                await asyncio.sleep(_jitter_delay(1.5 + attempt * 1.5, 1.2))
            continue
        ...
        if isinstance(result, dict) and result.get("_error"):
            ...
            if http_status == -1:
                ...
                if attempt < max_retries - 1:
                    await asyncio.sleep(_jitter_delay(2.0 + attempt * 2.0, 1.5))
                continue
            if http_status >= 400:
                logger.warning(f"[DouyinAPI] HTTP {http_status}: {endpoint}")
                if http_status in (408, 425, 429, 500, 502, 503, 504):
                    await asyncio.sleep(_jitter_delay(4.0 + attempt * 3.0, 2.0))
                    continue
                return None
...
async def get_user_profile(page, sec_user_id: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "avatar_url": _first_url_from_avatar_fields(user),
...
async def collect_douyin_data(
    ...
    while page_num < 1000:
        ...
        # 翻页间隔（避免触发频率限制）
        wait_seconds = max(float(sleep_sec or 0), 1.8) + random.uniform(0.4, 1.8)
    ...
    if all_awemes:
        ...
        if not result.metrics.get("_avatar"):
            for aweme in all_awemes:
                author_avatar = _first_url_from_avatar_fields(aweme.get("author", {}))
                if author_avatar:
                    result.metrics["_avatar"] = author_avatar
                    break
...
async def collect_target_user(
    ...
    while page_num < 1000:
        ...
        if str(next_cursor) == cursor_key or str(next_cursor) in seen_cursors:
            break
        cursor = next_cursor
        await asyncio.sleep(2.2 + random.uniform(0.4, 1.6))
