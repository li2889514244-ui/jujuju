...
function isTokenExpired(token: string): boolean {
  return isJwtExpiringWithin(token, 60)
}
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        meta: { title: '微信小店', icon: 'Shop', section: '商业转化', roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN', 'MANAGER', 'GROUP_LEADER'] },
      },
      {
        path: 'doudian',
        name: 'Doudian',
        component: () => import('@/views/monetization/DoudianView.vue'),
        meta: { title: '抖店', icon: 'ShoppingCart', section: '商业转化', roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN', 'MANAGER', 'GROUP_LEADER'] },
      },
      {
        path: 'performance-ladder',
        name: 'PerformanceLadder',
        component: () => import('@/views/monetization/PerformanceLadderView.vue'),
        meta: { title: '业绩天梯', icon: 'Aim', section: '商业转化', roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN', 'MANAGER', 'GROUP_LEADER'] },
      },
      {
        path: 'doudian/source-detail',
        name: 'DoudianSourceDetail',
        component: () => import('@/views/monetization/DoudianSourceDetailView.vue'),
        meta: { title: '抖店出单来源详情', hidden: true, roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN', 'MANAGER', 'GROUP_LEADER'] },
      },
      {
        path: 'monetization/source-detail',
        name: 'WechatSourceDetail',
        component: () => import('@/views/monetization/WechatSourceDetailView.vue'),
        meta: { title: '出单来源详情', hidden: true, roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN', 'MANAGER', 'GROUP_LEADER'] },
      },
      {
        path: 'team',
        name: 'Team',
      ...
      },
      {
        ...
        meta: { title: '权限管理', hidden: true, roles: ['SUPER_ADMIN', 'OWNER', 'ADMIN'] },
      },
      {
        path: 'platforms',
        name: 'Platforms',
      ...
      {
        path: 'system-health',
        name: 'SystemHealthCenter',
        component: () => import('@/views/admin/SystemHealthCenterView.vue'),
        meta: {
          title: '系统健康中心',
          icon: 'Monitor',
          section: '系统',
...
router.beforeEach((to, _from, next) => {
  ...
  const isFeishuCallback =
    to.name === 'Login' && typeof to.query.code === 'string' && typeof to.query.state === 'string'
