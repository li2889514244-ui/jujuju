  ...
  <div class="login">
    ...
    <aside class="login__brand">
      <div class="login__brand-inner">
        ...
        <p class="login__footer-note">© {{ currentYear }} 披星云 · 以简驭繁</p>
      </div>
    </aside>
          ...
          <div v-if="activeTab === 'login'" key="login">
            ...
            >
              <el-form-item prop="identifier">
                <el-input v-model="loginForm.identifier" placeholder="邮箱 / 手机号" :prefix-icon="Message" />
              </el-form-item>
...
const registrationEnabled =
  import.meta.env.DEV || import.meta.env.VITE_PUBLIC_REGISTRATION_ENABLED === 'true'
...
onMounted(() => {
  ...
  if (route.query.companion_feishu === '1') {
    handleFeishuLogin()
  }
})
...
function isValidCompanionCallback(value: string): boolean {
  try {
    ...
    return (
      ...
      url.port === '5409' &&
      url.pathname === '/api/feishu/complete'
    )
  } catch {
    return false
  }
...
function companionRedirectPath(): string | null {
  if (route.query.companion_feishu !== '1') return null
  const sessionId = firstQueryValue(route.query.companion_session).trim()
  const callback = firstQueryValue(route.query.companion_callback).trim()
  if (!sessionId || !isValidCompanionCallback(callback)) return null
...
async function completeCompanionFeishuLogin(data: {
  accessToken: string
  refreshToken: string
  user: unknown
  ...
  if (target.pathname !== '/login' || target.searchParams.get('companion_feishu') !== '1') return false
  const sessionId = target.searchParams.get('companion_session') || ''
  const callback = target.searchParams.get('companion_callback') || ''
  if (!sessionId || !isValidCompanionCallback(callback)) {
    throw new Error('本地伴侣回调地址无效')
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(body?.error || '无法通知本地伴侣完成登录')
  }
  ElMessage.success('本地伴侣飞书登录成功')
  router.replace({ name: 'Login', query: { companion_done: '1' } })
  return true
}
...
async function handleFeishuLogin() {
  ...
  try {
    const redirect = companionRedirectPath() || (route.query.redirect as string) || '/dashboard'
    const res = await authApi.getFeishuUrl(redirect)
...
async function handleFeishuCallback(code: string, state: string) {
  feishuLoading.value = true
  try {
    const res = await authApi.feishuCallback({ code, state })
    if (await completeCompanionFeishuLogin(res.data, res.data.redirect)) return
    userStore.setLoginSession(res.data)
    ElMessage.success('飞书登录成功')
    const redirect = res.data.redirect || (route.query.redirect as string) || '/dashboard'
...
async function handleRegister() {
  if (!registrationEnabled) {
    ElMessage.warning('公开注册已关闭，请联系管理员开通账号')
