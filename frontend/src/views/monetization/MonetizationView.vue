  ...
  <div class="monetization">
    ...
    <div class="monetization__grid">
      ...
      <div class="card">
        <div class="card__header">
          <span>最近佣金单</span>
          <el-tag size="small" type="warning">微信小店</el-tag>
        </div>
      ...
      <div class="card">
        ...
        <div v-loading="loading" class="product-grid">
          <div v-if="sortedProducts.length === 0" class="empty-hint">暂无小店商品</div>
    ...
    series: [
      {
        ...
        data: entries.map((e) =>
          trendMetric.value === 'orders'
            ? e[1].orders
            : e[1][trendMetric.value as 'commission' | 'gmv'] / 100,
        ),
      },
    ],
    graphic:
        ...
        ? [
            {
              ...
              left: 'center',
              top: 'center',
              style: { text: '暂无数据', fontSize: 14, fill: '#8c8c8c' },
            },
          ]
        : undefined,
  }
...
async function loadAll() {
  ...
  try {
    // Load stores
    const storeRes = await wechatStoreApi.getStores()
    if (Array.isArray(storeRes.data)) stores.value = storeRes.data
...
async function loadStoreData() {
  ...
  try {
    const [ordRes, prodRes] = await Promise.all([
      wechatStoreApi.getOrders(activeStoreId.value, { page_size: 100 }),
...
async function submitManual() {
  if (!manual.value.date || !manual.value.platform) {
...
.order-item {
  ...
  &__status.is-settled {
    color: #6b9e6c;
    background: rgba(107, 158, 108, 0.15);
  }
  &__status.is-paid {
    color: #e0a030;
