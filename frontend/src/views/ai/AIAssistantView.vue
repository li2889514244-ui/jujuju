...
async function autoFillAnomalyData() {
  try {
    const res = await analyticsApi.getFollowerTrend({ days: 30 })
    const realValues = (res.data ?? [])
      .map((point) => Number(point.value))
      .filter((value) => Number.isFinite(value))

    if (realValues.length > 0) {
      anomalyForm.value.dataset = JSON.stringify(realValues)
      anomalyForm.value.metric = '粉丝数'
      anomalyResult.value = null
      return
    }

    anomalyForm.value.dataset = ''
    anomalyForm.value.metric = ''
    anomalyResult.value = null
    ElMessage.warning('暂无可用的真实粉丝趋势数据')
  } catch {
    anomalyForm.value.dataset = ''
    anomalyForm.value.metric = ''
    anomalyResult.value = null
