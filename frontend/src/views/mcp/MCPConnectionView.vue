        ...
        <div class="key-list">
          <div v-for="key in allKeys" :key="key.id" class="key-item">
            <div class="key-item__info">
              ...
              <span class="key-item__token">{{ maskToken(key.token) }}</span>
      ...
      <section class="config-panel">
        ...
        <el-tabs v-model="activeTab" class="config-tabs">
          <el-tab-pane label="Cursor" name="cursor">
            <pre class="code-block">{{ cursorConfigPreview }}</pre>
          ...
          <el-tab-pane label="Claude Desktop" name="claude">
            ...
            <pre class="code-block">{{ claudeConfigPreview }}</pre>
          ...
          <el-tab-pane label="Cline / VS Code" name="cline">
            <pre class="code-block">{{ clineConfigPreview }}</pre>
          ...
          <el-tab-pane label="SSE (Cherry Studio 等)" name="sse">
            ...
            <pre class="code-block">{{ sseConfigPreview }}</pre>
          ...
          <el-tab-pane label="通用 HTTP / curl" name="curl">
            ...
            <pre class="code-block">{{ curlExamplePreview }}</pre>
...
const cursorConfigPreview = computed(() => redactActiveToken(cursorConfig.value))
const claudeConfigPreview = computed(() => redactActiveToken(claudeConfig.value))
const clineConfigPreview = computed(() => redactActiveToken(clineConfig.value))
const sseConfigPreview = computed(() => redactActiveToken(sseConfig.value))
const curlExamplePreview = computed(() => redactActiveToken(curlExample.value))
...
function maskToken(token: string) {
  if (!token || token === '<MCP key>') return token || '<MCP key>'
  if (token.length <= 12) return `${token.slice(0, 2)}...${token.slice(-2)}`
  return `${token.slice(0, 6)}...${token.slice(-4)}`
}

function redactActiveToken(text: string) {
  const token = activeToken.value
  if (!token || token === '<MCP key>') return text
  return text.split(token).join(displayToken.value)
}
