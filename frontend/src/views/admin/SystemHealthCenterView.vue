<template>
  <div class="system-health">
    ...
    <el-card shadow="never" class="sh-hero" :class="`sh-hero--${overview?.overallStatus || 'UNKNOWN'}`">
    ...
    <el-row :gutter="12" class="sh-cards">
      <el-col v-for="card in healthCards" :key="card.key" :xs="24" :sm="12" :lg="4">
        <div class="sh-card">
          ...
          <div class="sh-card__status" :class="`is-${card.status}`">{{ statusLabel(card.status) }}</div>
        ...
        <el-card shadow="never">
          ...
          <el-table :data="incidents" size="small" empty-text="暂无故障记录">
            ...
            <el-table-column label="确认" width="110">
              <template #default="{ row }">
                <el-button v-if="!row.acknowledgedAt" size="small" @click="ackIncident(row.id)">我看过了</el-button>
      ...
      <el-tab-pane label="事件流" name="events">
        <el-card shadow="never">
          ...
          <el-table :data="events" size="small" empty-text="暂无事件">
            ...
            <el-table-column prop="requestId" label="Request ID" width="180" show-overflow-tooltip />
          </el-table>
        </el-card>
      </el-tab-pane>
...
<style scoped>
.system-health { padding: 16px 20px; }
.sh-header { display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; margin-bottom: 14px; }
.sh-header h2 { margin: 0 0 4px; font-size: 22px; font-weight: 700; }
.sh-header p { margin: 0; color: #8a8f99; font-size: 13px; }
.sh-hero { margin-bottom: 12px; border-radius: 12px; }
.sh-hero__status { font-size: 28px; font-weight: 800; }
.sh-hero__meta { display: flex; flex-wrap: wrap; gap: 12px; color: #8a8f99; margin-top: 6px; font-size: 12px; }
.sh-hero--HEALTHY { border-color: rgba(52, 199, 89, 0.45); }
.sh-hero--DEGRADED { border-color: rgba(230, 162, 60, 0.55); }
.sh-hero--INCIDENT, .sh-hero--CRITICAL { border-color: rgba(224, 80, 80, 0.65); }
.sh-cards { margin-bottom: 12px; }
.sh-cards .el-col { margin-bottom: 12px; }
.sh-card { min-height: 92px; border: 1px solid var(--el-border-color-light); border-radius: 10px; padding: 14px; background: var(--el-bg-color); }
.sh-card__name { color: #8a8f99; font-size: 12px; }
.sh-card__status { margin: 8px 0; font-size: 20px; font-weight: 800; }
.sh-card__status.is-HEALTHY { color: #34c759; }
.sh-card__status.is-DEGRADED { color: #e6a23c; }
.sh-card__status.is-INCIDENT, .sh-card__status.is-CRITICAL { color: #e05050; }
.sh-card__status.is-UNKNOWN { color: #8a8f99; }
.sh-card__desc { color: #8a8f99; font-size: 12px; }
.sh-section { margin-bottom: 12px; }
.sh-section__head { display: flex; align-items: center; gap: 8px; justify-content: space-between; }
.sh-tabs { margin-top: 12px; }
.sh-muted { color: #8a8f99; }
</style>
