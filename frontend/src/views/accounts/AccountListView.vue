<template>
  <div class="account-list">
    <div class="account-list__header">
      <div>
        <span class="section-label">账号矩阵</span>
        <h2>账号状态、分组和作品数据</h2>
        <p>点击账号详情，可查看单账号视频播放、点赞、评论、收藏和互动率。</p>
      </div>
      <router-link to="/dashboard" class="account-list__header-link">查看矩阵数据</router-link>
    </div>

    <!-- Filters -->
    <GlassCard class="account-list__filter">
      <el-form :inline="true" :model="accountStore.filter">
        <el-form-item label="平台">
          <el-select
            v-model="accountStore.filter.platform"
            placeholder="全部平台"
            clearable
            style="width: 140px"
          >
            <el-option label="全部" value="" />
            <el-option
              v-for="(label, key) in PLATFORM_LABELS"
              :key="key"
              :label="label"
              :value="key"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="分组">
          <el-select
            v-model="accountStore.filter.group"
            placeholder="全部分组"
            clearable
            style="width: 140px"
          >
            <el-option label="全部" value="" />
            <el-option v-for="g in accountStore.groups" :key="g.id" :label="g.name" :value="g.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="搜索">
          <el-input
            v-model="accountStore.filter.keyword"
            placeholder="搜索账号名称"
            clearable
            style="width: 200px"
          />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handleSearch">
            <el-icon><Search /></el-icon>查询
          </el-button>
          <el-button @click="handleReset">重置</el-button>
        </el-form-item>
      </el-form>
    </GlassCard>

    <!-- Action Bar -->
    <div class="account-list__actions">
      <el-button type="primary" @click="showAddDialog = true">
        <el-icon><Plus /></el-icon>接入账号
      </el-button>
      <el-button @click="showGroupDialog = true">
        <el-icon><FolderAdd /></el-icon>新建分组
      </el-button>
      <el-button @click="showGroupManage = true">
        <el-icon><Setting /></el-icon>管理分组
      </el-button>
      <el-tooltip :content="selectedIds.length ? '移动到分组' : '请先选择账号'" placement="top">
        <el-button :disabled="!selectedIds.length" @click="handleBatchMove">
          <el-icon><FolderOpened /></el-icon>批量移动
        </el-button>
      </el-tooltip>
      <el-tooltip :content="selectedIds.length ? '删除选中账号' : '请先选择账号'" placement="top">
        <el-button type="danger" :disabled="!selectedIds.length" @click="handleBatchDelete">
          <el-icon><Delete /></el-icon>批量删除
        </el-button>
      </el-tooltip>
      <el-tooltip
        :content="enhancedAccounts.length ? '导出为 CSV 文件' : '暂无账号数据'"
        placement="top"
      >
        <el-button :disabled="enhancedAccounts.length === 0" class="ml-auto" @click="exportCSV">
          <el-icon><Download /></el-icon>导出 CSV
        </el-button>
      </el-tooltip>
    </div>

    <!-- Account Card Grid -->
    <div v-loading="loading" class="account-grid">
      <div
        v-for="acc in enhancedAccounts"
        :key="acc.id"
        class="account-card"
        :class="{ 'account-card--selected': selectedIds.includes(acc.id) }"
        @click="toggleSelect(acc.id)"
      >
        <div class="account-card__check">
          <el-icon v-if="selectedIds.includes(acc.id)" :size="20" color="var(--color-accent)"
            ><CircleCheckFilled
          /></el-icon>
          <span v-else class="account-card__check-empty" />
        </div>
        <el-avatar
          :size="48"
          :src="acc.avatar"
          :style="{ background: acc.avatar ? '' : getPlatformColor(acc.platform), color: '#fff' }"
          >{{ acc.nickname?.charAt(0) }}</el-avatar
        >
        <div class="account-card__info">
          <span class="account-card__name">{{ acc.nickname }}</span>
          <PlatformBadge :platform="acc.platform" size="sm" />
          <span class="account-card__operator">
            主负责人：{{ accountPrimaryOperatorName(acc) }}
            <em v-if="(acc.collaboratorOperators?.length || 0) > 0">
              +{{ acc.collaboratorOperators?.length }}协同
            </em>
          </span>
        </div>
        <div class="account-card__stats">
          <span class="account-card__stat-value">{{ formatCompactNum(acc.followers || 0) }}</span>
          <span class="account-card__stat-label">粉丝</span>
        </div>
        <span v-if="acc.groupName" class="account-card__group">{{ acc.groupName }}</span>
        <span
          class="account-card__online"
          :class="'online--' + accountOnlineState(acc)"
          :title="acc.onlineReason || accountOnlineLabel(acc)"
        >
          <i class="account-card__online-dot" />
          {{ accountOnlineLabel(acc) }}
        </span>
        <span class="account-card__status" :class="'status--' + (acc.tokenStatus || 'unknown')">
          {{ tokenStatusLabel(acc) }}
        </span>
        <div class="account-card__actions">
          <el-button
            text
            type="primary"
            size="small"
            @click.stop="$router.push(`/accounts/${acc.id}`)"
            >详情</el-button
          >
          <el-button text type="primary" size="small" @click.stop="openOperatorDialog(acc)"
            >运营</el-button
          >
          <el-button text type="danger" size="small" @click.stop="handleDelete(acc.id)"
          >删除</el-button
          >
        </div>
      </div>
    </div>

    <div v-if="enhancedAccounts.length === 0 && !loading" class="empty-state">
      <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
        <rect
          x="8"
          y="12"
          width="48"
          height="40"
          rx="6"
          stroke="var(--color-accent)"
          stroke-width="1.5"
          fill="none"
          opacity="0.5"
        />
        <circle
          cx="32"
          cy="30"
          r="8"
          stroke="var(--color-accent)"
          stroke-width="1.5"
          fill="none"
          opacity="0.5"
        />
        <path
          d="M18 48c4-6 10-10 16-10s12 4 16 10"
          stroke="var(--color-accent)"
          stroke-width="1.5"
          fill="none"
          stroke-linecap="round"
          opacity="0.5"
        />
      </svg>
      <h3>连接你的第一个账号</h3>
      <p>绑定社交媒体账号，开始矩阵管理与数据分析</p>
    </div>

    <!-- 批量操作浮动栏 -->
    <div v-if="selectedIds.length > 0" class="float-bar">
      <span class="float-bar__count">已选 {{ selectedIds.length }} 个账号</span>
      <div class="float-bar__actions">
        <el-button size="small" @click="handleBatchMove"
          ><el-icon><FolderOpened /></el-icon>批量移动</el-button
        >
        <el-button size="small" type="danger" @click="handleBatchDelete"
          ><el-icon><Delete /></el-icon>批量删除</el-button
        >
      </div>
    </div>

    <div v-if="accountStore.total > accountStore.filter.pageSize" class="account-list__pagination">
      <el-pagination
        v-model:current-page="accountStore.filter.page"
        v-model:page-size="accountStore.filter.pageSize"
        :total="accountStore.total"
        :page-sizes="[20, 50, 100]"
        layout="total, sizes, prev, pager, next, jumper"
        @size-change="handleSearch"
        @current-change="handleSearch"
      />
    </div>

    <!-- Group Dialog -->
    <el-dialog v-model="showGroupDialog" title="新建分组" width="400px">
      <el-form>
        <el-form-item label="分组名称">
          <el-input
            v-model="newGroupName"
            placeholder="请输入分组名称"
            @keyup.enter="handleCreateGroup"
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showGroupDialog = false">取消</el-button>
        <el-button type="primary" @click="handleCreateGroup">确定</el-button>
      </template>
    </el-dialog>

    <!-- Move Dialog -->
    <el-dialog v-model="showMoveDialog" title="移动到分组" width="400px">
      <el-form>
        <el-form-item label="目标分组">
          <el-select v-model="moveTargetGroup" placeholder="选择分组" style="width: 100%">
            <el-option v-for="g in accountStore.groups" :key="g.id" :label="g.name" :value="g.id" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showMoveDialog = false">取消</el-button>
        <el-button type="primary" @click="handleConfirmMove">确定移动</el-button>
      </template>
    </el-dialog>

    <!-- Group Management Dialog -->
    <el-dialog v-model="showGroupManage" title="管理分组" width="400px">
      <el-table :data="accountStore.groups" stripe size="small">
        <el-table-column prop="name" label="分组名称" />
        <el-table-column prop="_count.accounts" label="账号数" width="80" />
        <el-table-column label="操作" width="80">
          <template #default="{ row }">
            <el-popconfirm title="确定删除此分组？" @confirm="handleDeleteGroup(row.id)">
              <template #reference>
                <el-button text type="danger" size="small">删除</el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
        <el-button @click="showGroupManage = false">关闭</el-button>
      </template>
    </el-dialog>

    <!-- Add Account Guide Dialog -->
    <el-dialog v-model="showAddDialog" title="接入账号" width="540px">
      <div class="add-dialog">
        <div class="add-dialog__intro">
          <p class="add-dialog__eyebrow">推荐方式</p>
          <p class="add-dialog__title">用桌面伴侣完成扫码登录</p>
          <p class="add-dialog__desc">
            桌面伴侣会在本机打开平台登录页，登录成功后自动同步账号状态。
          </p>
        </div>
        <div v-if="companionOnline" class="companion-online-box">
          <p class="companion-online-box__status">桌面伴侣已连接</p>
          <p class="companion-online-box__hint">选择要绑定的平台，随后在弹出的窗口里完成登录。</p>
          <div class="companion-online-box__btns">
            <el-button
              v-for="p in bindablePlatforms"
              :key="p.id"
              :type="p.type"
              size="small"
              @click="openCompanionScan(p.id)"
              >{{ p.icon }} {{ p.name }}</el-button
            >
          </div>
        </div>
        <template v-else>
          <div class="companion-offline-box">
            <p><strong>1. 下载并安装桌面伴侣</strong></p>
            <p>2. 打开桌面伴侣</p>
            <p>3. 回到这里选择平台</p>
            <p>4. 扫码登录后，账号会自动同步</p>
          </div>
          <el-button
            type="primary"
            size="large"
            class="download-btn"
            @click="handleDownloadCompanion"
          >
            <el-icon><Download /></el-icon>下载桌面伴侣
          </el-button>
        </template>
        <p class="text-caption">遇到特殊平台或调试场景时，也可以使用手动录入。</p>
      </div>
      <template #footer>
        <el-button @click="showAddDialog = false">关闭</el-button>
        <el-button @click="openManualAdd">手动录入</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="showOperatorDialog" title="账号运营人员" width="520px">
      <div v-loading="operatorLoading" class="operator-dialog">
        <div v-if="operatorAccount" class="operator-dialog__account">
          <el-avatar :size="40" :src="operatorAccount.avatar">
            {{ operatorAccount.nickname?.charAt(0) }}
          </el-avatar>
          <div>
            <strong>{{ operatorAccount.nickname }}</strong>
            <span>{{ PLATFORM_LABELS[operatorAccount.platform] || operatorAccount.platform }}</span>
          </div>
        </div>
        <el-form label-width="92px">
          <el-form-item label="主负责人">
            <el-select
              v-model="operatorForm.primaryUserId"
              filterable
              placeholder="选择主负责人"
              style="width: 100%"
            >
              <el-option
                v-for="member in organizationMembers"
                :key="member.id"
                :label="memberLabel(member)"
                :value="member.id"
              />
            </el-select>
          </el-form-item>
          <el-form-item label="协同运营">
            <el-select
              v-model="operatorForm.collaboratorUserIds"
              multiple
              filterable
              collapse-tags
              placeholder="选择协同运营"
              style="width: 100%"
            >
              <el-option
                v-for="member in collaboratorMemberOptions"
                :key="member.id"
                :label="memberLabel(member)"
                :value="member.id"
              />
            </el-select>
          </el-form-item>
        </el-form>
        <p class="operator-dialog__hint">
          主负责人会同步到账号所有者；协同运营用于后续看板、提醒和业绩归因。
        </p>
      </div>
      <template #footer>
        <el-button @click="showOperatorDialog = false">取消</el-button>
        <el-button type="primary" :loading="operatorSaving" @click="saveOperatorDialog">
          保存
        </el-button>
      </template>
    </el-dialog>
    <!-- Manual Add Dialog -->
    <ManualAddDialog v-model:visible="showManualDialog" @success="handleBindSuccess" />
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, computed } from 'vue'
import dayjs from 'dayjs'
import { ElMessage, ElMessageBox } from 'element-plus'
import { CircleCheckFilled } from '@element-plus/icons-vue'
import { useAccountStore } from '@/store/account'
import { useUserStore } from '@/store/user'
import { accountsApi } from '@/api/accounts'
import { teamsApi } from '@/api/teams'
import { PLATFORM_LABELS } from '@/types'
import type { Account, TeamMember } from '@/types'
import GlassCard from '@/components/common/GlassCard.vue'
import PlatformBadge from '@/components/common/PlatformBadge.vue'
import ManualAddDialog from '@/components/account/ManualAddDialog.vue'
import { accountOnlineLabel, accountOnlineState, formatCompactNum, tokenStatusLabel } from '@/utils/format'
import { readJsonOr } from '@/utils/http'
import { getPlatformColor } from '@/composables/usePlatform'
import { useCompanionUrl } from '@/composables/useCompanionUrl'

const accountStore = useAccountStore()
const userStore = useUserStore()

const selectedIds = ref<string[]>([])
const showGroupDialog = ref(false)
const newGroupName = ref('')
const showMoveDialog = ref(false)
const moveTargetGroup = ref('')
const showGroupManage = ref(false)
const showAddDialog = ref(false)
const showManualDialog = ref(false)
const showOperatorDialog = ref(false)
const companionOnline = ref(false)
const loading = ref(false)
const operatorLoading = ref(false)
const operatorSaving = ref(false)
const operatorAccount = ref<Account | null>(null)
const organizationMembers = ref<TeamMember[]>([])
const operatorForm = ref({
  primaryUserId: '',
  collaboratorUserIds: [] as string[],
})
const bindablePlatforms = [
  { id: 'douyin', name: '抖音', icon: 'DY', type: 'primary' as const },
  { id: 'xiaohongshu', name: '小红书', icon: 'RED', type: 'danger' as const },
  { id: 'kuaishou', name: '快手', icon: 'KS', type: 'warning' as const },
  { id: 'tencent', name: '视频号', icon: 'WX', type: 'success' as const },
]

const { healthCheck } = useCompanionUrl()
const companionDownloadUrl = '/downloads/pixingyun-mate-setup.exe'
const companionUpdateManifestUrl = '/companion-updates/latest.json'

function resolveApiUrl() {
  const raw = import.meta.env.VITE_API_BASE_URL || '/api/v1'
  if (/^https?:\/\//.test(raw)) return raw
  return `${window.location.origin}${raw}`
}

async function handleDownloadCompanion() {
  const opened = window.open('about:blank', '_blank', 'noopener')
  try {
    const response = await fetch(companionUpdateManifestUrl, { cache: 'no-cache' })
    const manifest = await response.json()
    const url =
      typeof manifest?.installer_url === 'string' && manifest.installer_url
        ? manifest.installer_url
        : companionDownloadUrl
    if (opened) {
      opened.location.href = url
    } else {
      window.location.href = url
    }
  } catch {
    if (!opened) {
      window.location.href = companionDownloadUrl
    }
  }
}

async function checkCompanion() {
  const url = await healthCheck()
  if (url) {
    fetch(`${url}/health`)
      .then((r) => readJsonOr<any>(r, {}))
      .then((d) => {
        companionOnline.value = d.status === 'ok'
      })
      .catch(() => {
        companionOnline.value = false
      })
  } else {
    companionOnline.value = false
  }
}

async function openCompanionScan(platform: string) {
  const token = userStore.token
  const refreshToken = userStore.refreshToken
  const api = resolveApiUrl()
  const companionUrl = await healthCheck()
  if (!companionUrl) {
    ElMessage.error('桌面伴侣未启动，请先打开桌面伴侣')
    return
  }
  try {
    const resp = await fetch(`${companionUrl}/api/scan-bind/trigger`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ platform, token, refreshToken, api_url: api }),
    })
    if (!resp.ok) throw new Error('HTTP ' + resp.status)
    const data = await readJsonOr<any>(resp, { code: -1, msg: '发送失败' })
    if (data.code === 0) {
      ElMessage.success('扫码窗口已打开。完成扫码后，伴侣会继续做初始采集和同步；完成前不算绑定成功。')
    } else {
      ElMessage.error(data.msg || '发送失败')
    }
  } catch {
    ElMessage.error('桌面伴侣未启动，请先打开桌面伴侣')
  }
}

const enhancedAccounts = computed(() => accountStore.accounts)
const collaboratorMemberOptions = computed(() =>
  organizationMembers.value.filter((member) => member.id !== operatorForm.value.primaryUserId),
)

onMounted(() => {
  accountStore.fetchAccounts()
  accountStore.fetchGroups()
  checkCompanion()
})

async function handleSearch() {
  accountStore.filter.page = 1
  accountStore.setFilter(accountStore.filter)
  await accountStore.fetchAccounts()
}

function handleReset() {
  accountStore.filter.platform = ''
  accountStore.filter.group = ''
  accountStore.filter.keyword = ''
  accountStore.filter.page = 1
  handleSearch()
}

function toggleSelect(id: string) {
  const idx = selectedIds.value.indexOf(id)
  if (idx >= 0) {
    selectedIds.value.splice(idx, 1)
  } else {
    selectedIds.value.push(id)
  }
}

function memberName(member: Partial<TeamMember> & { name?: string; username?: string }) {
  return member.name || member.username || member.email || member.id || '未命名成员'
}

function memberLabel(member: TeamMember) {
  const name = memberName(member)
  return member.email ? `${name}（${member.email}）` : name
}

function accountPrimaryOperatorName(account: Account) {
  const primary = account.primaryOperator?.user
  return primary?.name || primary?.username || primary?.email || '未设置'
}

async function loadOrganizationMembers() {
  if (organizationMembers.value.length > 0) return
  const res = await teamsApi.getMembers('')
  organizationMembers.value = res.data
}

async function openOperatorDialog(account: Account) {
  operatorAccount.value = account
  showOperatorDialog.value = true
  operatorLoading.value = true
  try {
    const [operatorsRes] = await Promise.all([
      accountsApi.getOperators(account.id),
      loadOrganizationMembers(),
    ])
    const operators = operatorsRes.data.operators || []
    const primary =
      operators.find((operator) => operator.role === 'PRIMARY') ||
      account.primaryOperator ||
      null
    operatorForm.value.primaryUserId = primary?.userId || ''
    operatorForm.value.collaboratorUserIds = operators
      .filter((operator) => operator.role === 'COLLABORATOR')
      .map((operator) => operator.userId)
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || error?.message || '运营人员加载失败')
  } finally {
    operatorLoading.value = false
  }
}

async function saveOperatorDialog() {
  if (!operatorAccount.value) return
  if (!operatorForm.value.primaryUserId) {
    ElMessage.warning('请选择主负责人')
    return
  }
  operatorSaving.value = true
  try {
    await accountsApi.updateOperators(operatorAccount.value.id, {
      primaryUserId: operatorForm.value.primaryUserId,
      collaboratorUserIds: operatorForm.value.collaboratorUserIds.filter(
        (id) => id !== operatorForm.value.primaryUserId,
      ),
    })
    await accountStore.fetchAccounts()
    showOperatorDialog.value = false
    ElMessage.success('运营人员已保存')
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || error?.message || '运营人员保存失败')
  } finally {
    operatorSaving.value = false
  }
}

async function handleDelete(id: string) {
  await ElMessageBox.confirm(
    '删除该账号会同时永久删除作品明细、每日快照和趋势历史，且不可恢复。确定继续吗？',
    '删除账号',
    { type: 'warning', confirmButtonText: '永久删除', cancelButtonText: '取消' },
  )
  await accountStore.deleteAccount(id)
  ElMessage.success('删除成功')
}

async function handleBatchDelete() {
  await ElMessageBox.confirm(
    `删除选中的 ${selectedIds.value.length} 个账号会同时永久删除作品明细、每日快照和趋势历史，且不可恢复。确定继续吗？`,
    '批量删除账号',
    { type: 'warning', confirmButtonText: '永久删除', cancelButtonText: '取消' },
  )
  const results = await Promise.allSettled(selectedIds.value.map((id) => accountsApi.delete(id)))
  const successCount = results.filter((r) => r.status === 'fulfilled').length
  const failCount = results.filter((r) => r.status === 'rejected').length
  if (failCount === 0) {
    ElMessage.success(`批量删除成功，共 ${successCount} 个`)
  } else {
    ElMessage.warning(`删除完成：成功 ${successCount} 个，失败 ${failCount} 个`)
  }
  accountStore.fetchAccounts()
}

async function handleBatchMove() {
  if (!selectedIds.value.length) {
    ElMessage.warning('请先选择账号')
    return
  }
  moveTargetGroup.value = ''
  showMoveDialog.value = true
}

async function handleConfirmMove() {
  if (!moveTargetGroup.value) {
    ElMessage.warning('请选择目标分组')
    return
  }
  try {
    await accountsApi.moveToGroup(selectedIds.value, moveTargetGroup.value)
    showMoveDialog.value = false
    selectedIds.value = []
    await accountStore.fetchAccounts()
    ElMessage.success('移动成功')
  } catch (e: any) {
    ElMessage.error(e?.response?.data?.message || e?.message || '移动失败')
  }
}

async function handleCreateGroup() {
  if (!newGroupName.value.trim()) {
    ElMessage.warning('请输入分组名称')
    return
  }
  try {
    await accountsApi.createGroup(newGroupName.value.trim())
    showGroupDialog.value = false
    newGroupName.value = ''
    await accountStore.fetchGroups()
    ElMessage.success('创建成功')
  } catch (e: any) {
    ElMessage.error(e?.response?.data?.message || e?.message || '创建失败')
  }
}

async function exportCSV() {
  const res = await accountsApi.getList({
    ...accountStore.filter,
    page: 1,
    pageSize: 10000,
  })
  const exportData = res.data as any
  const allAccounts = (exportData.items || exportData.accounts || exportData.list || []) as any[]
  const headers = ['账号', '平台', '分组', '状态']
  const rows = allAccounts.map((r: any) => [
    r.nickname,
    r.platform,
    r.groupName || '-',
    r.tokenStatus === 'valid'
      ? '已连接'
      : r.tokenStatus === 'expiring_soon'
        ? '即将过期'
        : r.tokenStatus === 'expired'
          ? '已失效'
          : r.hasCookies
            ? '在线'
            : '待授权',
  ])
  const csvCell = (value: unknown) => `"${String(value ?? '').replace(/"/g, '""')}"`
  const csv = [
    headers.map(csvCell).join(','),
    ...rows.map((r: any[]) => r.map(csvCell).join(',')),
  ].join('\n')
  const blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `账号列表_${dayjs().format('YYYY-MM-DD')}.csv`
  a.click()
  URL.revokeObjectURL(url)
}

async function handleDeleteGroup(id: string) {
  try {
    await accountsApi.deleteGroup(id)
    await accountStore.fetchGroups()
    ElMessage.success('分组已删除')
  } catch (e: any) {
    ElMessage.error(e?.message || '删除失败')
  }
}

function handleBindSuccess() {
  ElMessage.success('账号绑定和初始采集已完成')
  accountStore.fetchAccounts()
}

function openManualAdd() {
  showManualDialog.value = true
  showAddDialog.value = false
}
</script>

<style lang="scss" scoped>
.account-list {
  display: flex;
  flex-direction: column;
  gap: $space-6;

  &__header {
    display: flex;
    align-items: flex-end;
    justify-content: space-between;
    gap: $space-5;
    padding: $space-8;
    border: 1px solid $border-base;
    border-radius: $radius-xl;
    background:
      radial-gradient(circle at 8% 10%, rgba($accent-500, 0.14), transparent 40%), $bg-elevated;
    box-shadow: $shadow-sm;
    position: relative;
    overflow: hidden;

    @media (max-width: 768px) {
      padding: $space-5;
    }
    @media (max-width: 480px) {
      padding: $space-4;
    }

    h2 {
      margin: $space-3 0;
      font-size: $text-h1;
      letter-spacing: -0.025em;
      font-weight: 600;
      color: $text-primary;
    }

    p {
      max-width: 620px;
      color: $text-secondary;
      margin: 0;
      font-size: $text-body;
      line-height: 1.6;
    }
  }

  &__header-link {
    position: relative;
    z-index: 1;
    padding: 9px 16px;
    border: 1px solid $border-strong;
    border-radius: $radius-full;
    background: rgba($accent-500, 0.08);
    color: $accent-400;
    font-size: $text-xs;
    font-weight: 600;
    letter-spacing: 0.01em;
    text-decoration: none;
    transition: all 0.2s $ease-out;

    &:hover {
      color: $bg-deep;
      background: $accent-500;
      border-color: $accent-500;
    }
  }

  &__filter {
    border-radius: $radius-lg;

    :deep(.el-form--inline .el-form-item) {
      margin-right: $space-3;
    }
  }

  &__actions {
    display: flex;
    gap: $space-2;
    flex-wrap: wrap;
    align-items: center;
    padding: $space-3;
    border: 1px solid $border-subtle;
    border-radius: $radius-lg;
    background: rgba(255, 255, 255, 0.015);

    .ml-auto {
      margin-left: auto;
    }
  }

  &__pagination {
    display: flex;
    justify-content: center;
    padding-top: $space-6;
  }
}

@media (max-width: 768px) {
  .account-list__header {
    align-items: flex-start;
    flex-direction: column;
  }
}

// === Account card grid ===
.account-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));
  gap: $space-4;

  @media (max-width: 768px) {
    grid-template-columns: repeat(auto-fill, minmax(160px, 1fr));
    gap: $space-3;
  }
  @media (max-width: 480px) {
    grid-template-columns: 1fr;
  }
}

.account-card {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: $space-3;
  min-height: 260px;
  padding: $space-6 $space-5 $space-5;
  background: $bg-elevated;
  border: 1px solid $border-base;
  border-radius: $radius-xl;
  box-shadow: $shadow-sm;
  cursor: pointer;
  transition:
    transform 0.25s $ease-out,
    border-color 0.2s $ease-out,
    box-shadow 0.25s $ease-out;
  text-align: center;
  position: relative;
  overflow: hidden;

  @media (max-width: 768px) {
    min-height: 200px;
    padding: $space-4 $space-3;
    gap: $space-2;
  }

  &::before {
    content: '';
    position: absolute;
    top: 0;
    right: 0;
    left: 0;
    height: 2px;
    background: linear-gradient(90deg, transparent, $accent-500, transparent);
    opacity: 0;
    transition: opacity 0.25s $ease-out;
  }

  &:hover {
    transform: translateY(-2px);
    border-color: $border-strong;
    box-shadow: $shadow-md;

    &::before {
      opacity: 0.6;
    }
  }

  &--selected {
    border-color: $accent-500;
    background: linear-gradient(180deg, rgba($accent-500, 0.06), transparent 60%), $bg-elevated;

    &::before {
      opacity: 1;
    }
  }

  &__check {
    position: absolute;
    top: $space-3;
    right: $space-3;
  }
  &__check-empty {
    width: 18px;
    height: 18px;
    border-radius: 50%;
    border: 1.5px solid $border-strong;
    display: block;
    transition: border-color 0.15s $ease-out;
  }
  &:hover &__check-empty {
    border-color: $accent-400;
  }

  &__info {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 6px;
    min-width: 0;
    width: 100%;
  }
  &__name {
    font-size: 16px;
    font-weight: 600;
    color: $text-primary;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
    max-width: 100%;
    letter-spacing: -0.01em;
  }
  &__operator {
    max-width: 100%;
    color: $text-tertiary;
    font-size: $text-micro;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;

    em {
      color: $accent-300;
      font-style: normal;
      margin-left: 4px;
    }
  }
  &__stats {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 0;
  }
  &__stat-value {
    font-family: $font-mono;
    font-size: 28px;
    font-weight: 600;
    color: $text-primary;
    font-feature-settings: 'tnum' 1;
    font-variant-numeric: tabular-nums;
    letter-spacing: -0.02em;
    line-height: 1.1;

    @media (max-width: 768px) {
      font-size: 22px;
    }
  }
  &__stat-label {
    font-size: $text-micro;
    color: $text-tertiary;
    font-weight: 500;
    letter-spacing: 0.08em;
    text-transform: uppercase;
    margin-top: 2px;
  }
  &__group {
    font-size: $text-micro;
    color: $text-secondary;
    background: rgba(255, 255, 255, 0.05);
    border: 1px solid $border-subtle;
    padding: 3px 10px;
    border-radius: $radius-full;
  }
  &__online {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    font-size: $text-micro;
    font-weight: 600;
    padding: 3px 10px;
    border-radius: $radius-full;

    &.online--online {
      background: rgba($color-success, 0.12);
      color: $color-success;
    }

    &.online--offline {
      background: rgba($color-danger, 0.12);
      color: $color-danger;
    }
  }
  &__online-dot {
    width: 7px;
    height: 7px;
    border-radius: 50%;
    background: currentColor;
    box-shadow: 0 0 0 3px rgba(255, 255, 255, 0.06);
  }
  &__status {
    font-size: $text-micro;
    font-weight: 600;
    padding: 3px 10px;
    border-radius: $radius-full;
    &.status--valid {
      background: rgba($color-success, 0.12);
      color: $color-success;
    }
    &.status--expiring_soon {
      background: rgba($color-warning, 0.14);
      color: $color-warning;
    }
    &.status--expired,
    &.status--unknown {
      background: rgba($color-danger, 0.12);
      color: $color-danger;
    }
  }
  &__actions {
    display: flex;
    gap: $space-2;
    margin-top: auto;
  }
}

// === Add dialog ===
.add-dialog {
  display: flex;
  flex-direction: column;
  gap: $space-4;
  padding: $space-2 0;
  .add-dialog__title {
    font-size: $text-h3;
    color: $text-primary;
    font-weight: 600;
    margin: 0;
    letter-spacing: -0.01em;
  }
  .add-dialog__eyebrow {
    color: $accent-400;
    font-size: $text-micro;
    font-weight: 600;
    letter-spacing: 0.08em;
    margin: 0 0 4px;
    text-transform: uppercase;
  }
  .add-dialog__desc {
    color: $text-secondary;
    font-size: $text-body;
    margin: 6px 0 0;
    line-height: 1.6;
  }
}
.companion-online-box {
  background: rgba(255, 255, 255, 0.025);
  border: 1px solid $border-base;
  border-radius: $radius-md;
  padding: $space-4;
  .companion-online-box__status {
    color: $color-success;
    font-weight: 600;
    margin-bottom: $space-2;
    font-size: $text-body;
  }
  .companion-online-box__hint {
    color: $text-secondary;
    font-size: $text-body;
    margin-bottom: $space-3;
  }
  .companion-online-box__btns {
    display: flex;
    gap: $space-2;
    flex-wrap: wrap;
    justify-content: flex-start;
  }
}
.companion-offline-box {
  background: rgba(255, 255, 255, 0.025);
  border: 1px solid $border-base;
  border-radius: $radius-md;
  padding: $space-4;
  text-align: left;
  p {
    margin: 6px 0;
    font-size: $text-body;
    color: $text-secondary;
    line-height: 1.6;
  }
}
.download-btn {
  margin-bottom: $space-4;
}

.operator-dialog {
  display: flex;
  flex-direction: column;
  gap: $space-4;

  &__account {
    display: flex;
    align-items: center;
    gap: $space-3;
    padding: $space-3;
    border: 1px solid $border-base;
    border-radius: $radius-md;
    background: rgba(255, 255, 255, 0.03);

    strong,
    span {
      display: block;
    }

    strong {
      color: $text-primary;
      font-size: $text-body;
    }

    span {
      color: $text-tertiary;
      font-size: $text-caption;
      margin-top: 2px;
    }
  }

  &__hint {
    color: $text-tertiary;
    font-size: $text-caption;
    line-height: 1.6;
    margin: 0;
  }
}

// === Float bar ===
.float-bar {
  position: fixed;
  bottom: 24px;
  left: 50%;
  transform: translateX(-50%);
  display: flex;
  align-items: center;
  gap: $space-5;
  background: rgba(15, 16, 24, 0.92);
  border: 1px solid $border-strong;
  border-radius: $radius-full;
  box-shadow: $shadow-lg;
  backdrop-filter: blur(20px) saturate(140%);
  -webkit-backdrop-filter: blur(20px) saturate(140%);
  padding: $space-3 $space-5;
  z-index: 100;

  @media (max-width: 768px) {
    left: 12px;
    right: 12px;
    transform: none;
    bottom: 12px;
    padding: $space-2 $space-4;
    gap: $space-3;
    border-radius: $radius-lg;
  }
  &__count {
    font-size: $text-body;
    font-weight: 500;
    color: $text-primary;
    font-feature-settings: 'tnum' 1;
    font-variant-numeric: tabular-nums;
  }
  &__actions {
    display: flex;
    gap: $space-2;
  }
}

// === Empty state ===
.empty-state {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: $space-3;
  padding: $space-16 0;
  text-align: center;
  h3 {
    font-size: $text-h3;
    font-weight: 600;
    color: $text-primary;
    margin: 0;
    letter-spacing: -0.01em;
  }
  p {
    font-size: $text-body;
    color: $text-tertiary;
    margin: 0;
  }
}
</style>


