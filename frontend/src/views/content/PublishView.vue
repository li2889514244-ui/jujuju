  ...
  <div class="publish-wizard">
    ...
    <div v-if="step === 0" class="publish-wizard__body">
      <el-card shadow="hover">
        ...
        <el-tabs v-model="contentMode">
          <el-tab-pane label="选择已有内容" name="existing">
            <el-table :data="contentStore.contents" stripe highlight-current-row @current-change="selectContent" max-height="400">
          ...
          <el-tab-pane label="新建内容" name="new">
            <el-form :model="contentForm" label-width="80px">
              <el-form-item label="标题">
                <el-input v-model="contentForm.title" placeholder="视频/图文标题" maxlength="50" show-word-limit />
              </el-form-item>
              <el-form-item label="描述">
                <el-input v-model="contentForm.description" type="textarea" :rows="3" placeholder="内容描述" maxlength="500" show-word-limit />
              </el-form-item>
              <el-form-item label="标签">
                <el-select v-model="contentForm.tags" multiple filterable allow-create placeholder="输入标签" style="width:100%" />
              </el-form-item>
            </el-form>
          </el-tab-pane>
        </el-tabs>
      ...
      <div class="publish-wizard__actions">
        ...
        <el-button type="primary" @click="step = 1" :disabled="!selectedContentId && contentMode === 'existing'">
                ...
                <div v-for="acc in p.accounts" :key="acc.id" class="account-item">
                  <el-checkbox :value="acc.id" :disabled="acc.cookieStatus !== 'valid'">
                    <div class="account-item__info">
                      <el-avatar :size="28" :src="acc.avatar">{{ acc.nickname?.charAt(0) }}</el-avatar>
                      <span>{{ acc.nickname }}</span>
                      <span class="account-item__followers">{{ formatNum(acc.followers) }} 粉丝</span>
                    </div>
                  </el-checkbox>
                  <!-- Per-account custom title -->
                  <el-input v-model="customTitles[acc.id]" size="small" placeholder="可为此账号自定义标题" style="width:200px;margin-left:12px" />
                </div>
              </el-checkbox-group>
            </div>
            <div v-else class="account-list__empty">
              暂无{{ p.label }}账号 — <el-button text type="primary" size="small" @click="$router.push('/accounts')">添加账号</el-button>
            </div>
          </el-collapse-item>
        </el-collapse>

        <el-form-item v-if="publishMode === 'scheduled'" label="发布时间" style="margin-top:16px">
          <el-date-picker v-model="scheduledAt" type="datetime" placeholder="选择发布时间" format="YYYY-MM-DD HH:mm" style="width:280px" />
        </el-form-item>
      </el-card>
    ...
    <div v-if="step === 2" class="publish-wizard__body">
      <el-card shadow="hover">
        ...
        <el-descriptions :column="2" border size="small" class="review-desc">
          <el-descriptions-item label="内容标题">{{ contentForm.title || selectedContent?.title || '新内容' }}</el-descriptions-item>
          <el-descriptions-item label="发布方式">{{ publishMode === 'now' ? '立即发布' : dayjs(scheduledAt).format('YYYY-MM-DD HH:mm') }}</el-descriptions-item>
          <el-descriptions-item label="目标账号">{{ totalSelectedAccounts }} 个</el-descriptions-item>
        </el-descriptions>

        <div class="review-accounts">
          <el-tag v-for="acc in selectedAccountDetails" :key="acc.id" size="small" closable @close="removeAccount(acc)" style="margin:4px">
      ...
      <div class="publish-wizard__actions">
        <el-button @click="step = 1"><el-icon><ArrowLeft /></el-icon> 上一步</el-button>
      ...
      <el-card v-if="publishResults.length > 0" shadow="hover" class="publish-wizard__results">
        ...
        <div v-for="r in publishResults" :key="r.accountId" class="result-item" :class="'result-' + r.status">
...
import { ArrowLeft, ArrowRight, Promotion, CircleCheckFilled, CircleCloseFilled } from '@element-plus/icons-vue'
  ...
  try {
    ...
    if (Array.isArray(data)) {
      publishResults.value = data.map((r: any) => ({
        accountId: r.accountId, status: r.status || 'success', nickname: r.accountNickname || '', error: r.errorMessage,
      }))
    } else {
...
.publish-wizard {
  &__steps { margin-bottom: 28px; }
  &__body { max-width: 900px; margin: 0 auto; }
  &__actions { display: flex; justify-content: space-between; margin-top: 20px; }
  &__accounts-card { margin-bottom: 0; }
  &__results { margin-top: 20px; }
}

.step2-header { display: flex; justify-content: space-between; align-items: center; }
...
.review-accounts { margin-top: 8px; }
