...
export const analyticsApi = {
  getOverview(params?: { groupId?: string; teamId?: string; platform?: string }) {
  ...
  getPlatformStats(params?: { groupId?: string; platform?: string }) {
  ...
  getComparison(params?: { groupId?: string; platform?: string }) {
    return get<{
  ...
  async triggerRealDataCollection() {
    ...
    try {
      ...
      const json = await readJsonOr(resp, {})
      return json
    } catch {
      throw new Error('桌面伴侣未启动，请先打开桌面伴侣')
    }
