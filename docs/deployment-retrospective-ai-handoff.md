...
## 2026-08-10 Pixingyun Mate 3.2.51 复盘

这次事件从一个采集异常开始，最后暴露出三类发布交接问题：UI 选择逻辑、Windows 本机安装状态、以及 companion 两包发布流程。

### 现象

- 用户截图显示微信视频号数据中心的周期控件是紧凑版 `日` / `周` / `月`。
- 采集 `近30天` 时没有实际点击 `月`，导致月维度数据可能沿用旧周期或默认周期。
- 用户本机桌面图标显示异常，且自启动看起来不稳定。
- 第一次云端发布只上传了 portable 更新 exe 和 manifest，漏掉了完整安装包。

### 根因

- `desktop-companion/companion_metrics.py` 里的周期选择逻辑把“卡片内包含目标文字”误判为“目标周期已选中”。紧凑版 UI 会同时显示 `日` / `周` / `月`，这个判断不成立。
- 代码只按 `近30天` 等长标签找控件，没有把 `近30天` 映射到紧凑 UI 的 `月`。
- PyInstaller spec 缺少 icon 配置，导致重新打包后 exe 图标可能不是预期品牌图标。
- 本机启动项指向源码 `launcher.py`，不是安装目录 exe；桌面图标还受 Windows 图标缓存影响。
- 发布时只关注了自动更新 manifest 指向的包，忘了网站/manual install 仍依赖 full installer 和稳定 alias。

### 修复动作

- Pixingyun Mate 升级到 `3.2.51`。
- 增加 period alias：`昨日数据/日`、`近7天/周`、`近30天/月`。
- 移除“看到目标文字就返回成功”的早退逻辑，改为在目标指标卡内查找并点击可见周期控件。
- PyInstaller spec 加入 `icon='app_icon.ico'`。
- 本机 autostart 改为直接启动 `D:\Pixingyun\pixingyun-mate.exe`，桌面快捷方式改为安装目录下带版本的新 icon 文件。
- 发布 portable 更新包、manifest、versioned manifest、full installer、stable installer alias，并逐个做公网验证。

### 以后发布检查清单

Companion 发布必须同时完成：

- `py -3 -m py_compile` 检查改动 Python 文件。
- PyInstaller 构建 `dist\pixingyun-mate.exe`。
- Windows associated icon 抽取检查，确认 exe 图标不是默认/旧图标。
- 本机 `/health` 返回目标版本。
- 上传 `pixingyun-mate-portable-{version}.exe`。
- 上传 `pixingyun-mate-setup-{version}.exe`。
- 覆盖稳定 installer alias `pixingyun-mate-setup.exe`。
- 更新并上传 `companion-updates/latest.json` 和 `latest-{version}.json`。
- 公网 `HEAD` 验证 portable、versioned setup、stable setup 的 `200` 和 `Content-Length`。
- 本机 `/api/update/check` 在上传后返回 `available=false`。
- 远端或公网下载 hash 对比 manifest/hash 记录。
- 更新 `docs/deployment-log.md`、`LATEST_DEPLOYMENT.md`、`docs/project-memory.md`。

### 交接提醒

不要把 “manifest 已经是最新版” 等同于 “所有用户路径已发布”。老用户走 in-app update，重装/新电脑走 full installer，官网按钮可能走 stable alias。这三条路径要一起验。
