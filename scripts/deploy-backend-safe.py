...
not touch frontend dist files or nginx.
"""
...
def create_backend_archive() -> Path:
    ...
    with tarfile.open(out, "w:gz") as tar:
        ...
        tar.add(ROOT / "package.json", arcname="root-package.json")
        ...
        tar.add(FRONTEND / "package.json", arcname="frontend-package.json")
    print(f"archive={out}")
    return out
...
def deploy_archive(client, archive: Path, remote_backend: str, pm2_app: str, *, migrate: bool, install_deps: bool) -> None:
    ...
    install_cmd = "cd /opt/matrixflow && npm ci --omit=dev --ignore-scripts --legacy-peer-deps" if install_deps else "true"
    manifest_sync_cmd = (
        textwrap.dedent(
            """
            cp -a /opt/matrixflow/package.json "$BACKUP/root-package.json" 2>/dev/null || true
            cp -a /opt/matrixflow/package-lock.json "$BACKUP/package-lock.json" 2>/dev/null || true
            cp -a /opt/matrixflow/frontend/package.json "$BACKUP/frontend-package.json" 2>/dev/null || true
            cp -a "$TMP/root-package.json" /opt/matrixflow/package.json
            cp -a "$TMP/package-lock.json" /opt/matrixflow/package-lock.json
            cp -a "$TMP/frontend-package.json" /opt/matrixflow/frontend/package.json
            """
        ).strip()
        if install_deps
        else "true"
    ...
    command = textwrap.dedent(
        ...
        cp -a "$REMOTE_BACKEND/.env" "$BACKUP/.env" 2>/dev/null || true
        ...
        cd "$REMOTE_BACKEND"
        touch .env
        if grep -q '^JWT_REFRESH_EXPIRES=' .env; then
          sed -i 's/^JWT_REFRESH_EXPIRES=.*/JWT_REFRESH_EXPIRES=365d/' .env
        else
          printf '\\nJWT_REFRESH_EXPIRES=365d\\n' >> .env
        fi
        if ! grep -q '^JWT_ACCESS_EXPIRES=' .env; then
          printf 'JWT_ACCESS_EXPIRES=15m\\n' >> .env
        fi
        printf 'jwt_env=%s\\n' "$(grep -E '^JWT_(ACCESS|REFRESH)_EXPIRES=' .env | tr '\\n' ' ')"
        test "$(grep '^JWT_REFRESH_EXPIRES=' .env | tail -n1 | cut -d= -f2-)" = "365d"
        node - <<'NODE'
        const fs = require('fs')
        const required = [
          'dist/config/jwt.config.js',
          'dist/modules/auth/auth.service.js',
          'dist/modules/scan-bind/scan-bind.module.js',
        ]
        for (const file of required) {{
          if (!fs.existsSync(file)) throw new Error(`Missing ${{file}}`)
        }}
        const jwtConfig = fs.readFileSync('dist/config/jwt.config.js', 'utf8')
        const authService = fs.readFileSync('dist/modules/auth/auth.service.js', 'utf8')
        const scanBind = fs.readFileSync('dist/modules/scan-bind/scan-bind.module.js', 'utf8')
        if (!jwtConfig.includes('365d') || !authService.includes('365d')) {{
          throw new Error('Deployed auth dist does not contain 365d refresh-token defaults')
        }}
        if (scanBind.includes("'7d'") || scanBind.includes('"7d"')) {{
          throw new Error('Deployed scan-bind dist still contains 7d JWT default')
        }}
        console.log('jwt_dist=365d defaults verified')
        NODE
        npx prisma generate
        {migrate_cmd}
        pm2 restart "$PM2_APP" --update-env
        for attempt in $(seq 1 18); do
          if curl -sf --max-time 8 http://localhost:3000/api/v1/health; then
            break
          fi
          if [ "$attempt" -eq 18 ]; then
            echo "Backend health did not recover after restart" >&2
            pm2 logs "$PM2_APP" --lines 30 --nostream 2>&1 || true
            exit 7
          fi
          sleep 3
        done
...
def print_plan(args: argparse.Namespace) -> None:
    ...
    if args.install_deps:
        print("6. Sync root/frontend/backend package manifests and package-lock, then reinstall production dependencies with npm ci --omit=dev --ignore-scripts --legacy-peer-deps.")
    else:
        print("6. Keep existing server node_modules.")
    print("7. Run npx prisma generate.")
    print("8. Ensure remote .env contains JWT_REFRESH_EXPIRES=365d before restart.")
    print("9. Assert deployed dist contains 365d refresh-token defaults.")
    print("10. Run prisma migrate deploy only if --migrate is passed; otherwise run migrate status.")
    print(f"11. Restart PM2 app {args.pm2_app}.")
    print(f"12. Verify {args.health_url}.")
    print("")
