...
export class ContentService {
  ...
  async create(dto: CreateContentDto, userId: string) {
    ...
    if (!account) throw new NotFoundException('关联账号不存在');
    if (account.userId !== userId) throw new ForbiddenException('无权为此账号创建内容');
  ...
  async findAll(params: { userId?: string; accountId?: string; status?: PostStatus; platform?: string; skip?: number; take?: number }) {
    const { userId, accountId, status, platform, skip = 0, take = 20 } = params;
    const where: Prisma.PostWhereInput = {};
    if (accountId) where.accountId = accountId;
    else if (userId) where.account = { userId };
  ...
  async findById(id: string, userId?: string) {
    ...
    if (userId && post.account.userId !== userId) {
      ...
      if (!user || !['OWNER', 'ADMIN'].includes(user.role)) throw new ForbiddenException('无权查看此内容');
    }
    return post;
  }
  ...
  async update(id: string, data: Partial<CreateContentDto>, userId: string) {
    ...
    if (!post) throw new NotFoundException('内容不存在');
    if (!['DRAFT', 'SCHEDULED'].includes(post.status)) throw new BadRequestException('只有草稿和定时状态的内容可以修改');
    if (post.account.userId !== userId) throw new ForbiddenException('无权修改此内容');
    ...
    if (data.publishAt !== undefined) { updateData.publishAt = data.publishAt ? new Date(data.publishAt) : null; updateData.status = data.publishAt ? 'SCHEDULED' : 'DRAFT'; }
  ...
  async remove(id: string, userId: string) {
    const post = await this.prisma.post.findUnique({ where: { id }, include: { account: true } });
    if (!post) throw new NotFoundException('内容不存在');
    if (post.account.userId !== userId) throw new ForbiddenException('无权删除此内容');
  ...
  async publish(contentId: string, userId: string) {
    ...
    if (!post) throw new NotFoundException('内容不存在');
    if (post.account.userId !== userId) throw new ForbiddenException('无权发布此内容');
  ...
  async updatePublishStatus(contentId: string, status: PostStatus, platformUrl?: string, errorMsg?: string) {
  ...
  async batchPublish(dto: { title: string; content: string; mediaUrls?: string[]; tags?: string[]; accountIds: string[]; publishAt?: string }, userId: string) {
    ...
    if (!accountIds || accountIds.length === 0) throw new BadRequestException('至少选择一个账号');
    ...
    if (accounts.length !== accountIds.length) throw new ForbiddenException('部分账号不属于当前用户');

    const posts = await Promise.all(accounts.map((account) =>
      this.prisma.post.create({
        data: { title: contentData.title, content: contentData.content, mediaUrls: contentData.mediaUrls || undefined, tags: contentData.tags || [], publishAt: contentData.publishAt ? new Date(contentData.publishAt) : null, status: contentData.publishAt ? 'SCHEDULED' : 'PUBLISHING', accountId: account.id },
        include: { account: { select: { id: true, platform: true, nickname: true } } },
      })
    ));
