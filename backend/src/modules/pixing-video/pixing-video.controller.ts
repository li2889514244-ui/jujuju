...
import { ServiceAuth } from '../../common/decorators/service-auth.decorator'
...
export class PixingVideoController {
  ...
  listTasks(@CurrentUser('id') userId: string, @Query('status') status?: string) {
    return this.service.listTasks(userId, status)
  }

  @Get('tasks/next')
  @ServiceAuth()
  @ApiOperation({ summary: '[本地服务] 获取下一个待处理任务（需 Service Token）' })
  getNextTask() {
  ...
  @Patch('tasks/:id')
  @ServiceAuth()
  @ApiOperation({ summary: '[本地服务] 更新任务状态（需 Service Token）' })
