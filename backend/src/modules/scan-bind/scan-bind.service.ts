 ...
 */
const PLATFORM_LOGIN_CONFIG: Record<
  string,
  {
    url: string
...
export class ScanBindService {
   ...
   *
   * 原因：服务器 IP（如阿里云数据中心）会被平台检测并封号。
   * 扫码绑定应通过本地伴侣（localhost:5409）完成，而非在服务器上运行。
   *
   * 如需在开发环境中使用，设置环境变量 ALLOW_SERVER_SCAN_BIND=true。
   */
  private assertNotProductionScan(): void {
    const isProd = this.configService.get<string>('NODE_ENV') === 'production'
    const allowOverride = this.configService.get<string>('ALLOW_SERVER_SCAN_BIND') === 'true'
    if (isProd && !allowOverride) {
      this.logger.error(
        '⚠ 拒绝在服务器上启动 Playwright 扫码绑定！' +
          '这会暴露服务器 IP 导致平台封号。' +
          '请通过本地伴侣（桌面伴侣）完成扫码绑定。' +
          '如确实需要在服务器上运行，设置 ALLOW_SERVER_SCAN_BIND=true。',
      )
      throw new Error('服务器端扫码绑定已被禁用。请使用桌面伴侣完成扫码绑定。')
    }
    this.logger.warn(
      '⚠ 服务端 Playwright 扫码绑定正在运行。' +
        '在生产环境中这会导致平台封号，请确保仅在开发环境使用。',
    )
  }

  async startScanSession(params: Omit<ScanSession, 'cancelled' | 'context' | 'page' | 'timer'>) {
    // 安全检查：生产环境拒绝在服务器上启动 Playwright
    this.assertNotProductionScan()
  ...
  private async pollQrCodeAndStatus(
    ...
    while (attempts < maxAttempts && !session.cancelled) {
      ...
      try {
        ...
        const isLoggedIn =
          indicator instanceof RegExp ? indicator.test(currentUrl) : currentUrl.includes(indicator)
        ...
        if (!qrSent) {
              ...
              try {
                ...
                if (
                  ...
                  if (
                    ...
                    qrElement = img
                    this.logger.log(
                      `Fallback QR via <img> #${i}: ${box.width}x${box.height} src=${src.substring(0, 50)}`,
                    )
                    break
                  }
                }
          ...
          if (qrElement) {
            ...
            const viewportShot = await page
              .screenshot({ type: 'png', clip: { x: 0, y: 0, width: 1280, height: 720 } })
              .catch(() => null)
            if (viewportShot) {
    ...
    try {
      ...
      switch (platform) {
        case 'DOUYIN':
          ...
          nickname = await page
            .$eval(
              '[class*="name"], [class*="nickname"], .account-name span',
              (el) => el.textContent?.trim() || '',
            )
            .catch(() => '')
          avatar = await page
            .$eval(
              '[class*="avatar"] img, .avatar-img img',
        ...
        case 'XIAOHONGSHU':
          ...
          nickname = await page
            .$eval(
              '[class*="name"], [class*="nickname"], .username',
              (el) => el.textContent?.trim() || '',
            )
            .catch(() => '')
          avatar = await page
        ...
        case 'KUAISHOU':
          await page.waitForTimeout(3000)
          nickname = await page
            .$eval('[class*="name"], [class*="nickname"]', (el) => el.textContent?.trim() || '')
            .catch(() => '')
          avatar = await page
            .$eval(
        ...
        case 'BILIBILI':
          await page.waitForTimeout(2000)
          nickname = await page
            .$eval('[class*="nickname"], .h-name', (el) => el.textContent?.trim() || '')
            .catch(() => '')
          avatar = await page
            .$eval(
              '[class*="avatar"] img, .h-avatar img',
              (el) => (el as HTMLImageElement).src || '',
            )
            .catch(() => '')
          platformUserId = await page.evaluate(() => {
        ...
        case 'WEIBO':
          await page.waitForTimeout(3000)
          nickname = await page
            .$eval('[class*="name"], .username', (el) => el.textContent?.trim() || '')
            .catch(() => '')
          avatar = await page
            .$eval('[class*="avatar"] img, .face img', (el) => (el as HTMLImageElement).src || '')
            .catch(() => '')
          platformUserId = await page.evaluate(() => {
        ...
        case 'WECHAT_VIDEO':
          await page.waitForTimeout(3000)
          nickname = await page
            .$eval('[class*="nickname"], [class*="name"]', (el) => el.textContent?.trim() || '')
            .catch(() => '')
          avatar = await page
            .$eval(
              '[class*="avatar"] img, [class*="head"] img',
  ...
  private async saveAccount(params: {
    ...
    const account = await this.prisma.account.upsert({
      ...
      update: {
        ...
        ...(this.isSuspiciousAccountNickname(params.platform, params.nickname) ? {} : { nickname }),
        avatar,
      },
      create: {
        platform: params.platform as any,
