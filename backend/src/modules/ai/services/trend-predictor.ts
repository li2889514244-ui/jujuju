...
export class TrendPredictorService {
  ...
  async predictTrend(dto: PredictTrendDto): Promise<TrendPrediction> {
    ...
    if (dto.historicalData) {
      try {
        const parsed = JSON.parse(dto.historicalData);
        if (Array.isArray(parsed)) {
          historicalValues = toFiniteNumberSeries(parsed);
          dataSource = 'real';
        } else {
          this.logger.warn('Invalid historical data JSON shape');
        }
      } catch {
  ...
  private async fetchRealMetrics(metric: string, platform: string | undefined, days: number): Promise<number[]> {
    try {
      ...
      for (const s of stats) {
        ...
        const parsed = Number((s as any)[metricField] || 0);
  ...
  async getMetricSummary(data: number[]): Promise<{
    ...
    const values = toFiniteNumberSeries(data);
    return {
      mean: mean(values),
