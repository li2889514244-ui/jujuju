...
import {
  ...
  mean,
  standardDeviation,
  toFiniteNumberSeries,
  type AnomalyPoint,
} from '../utils/data-analyzer';
...
export class AnomalyDetectorService {
  ...
  async detect(dto: DetectAnomalyDto): Promise<AnomalyReport> {
    ...
    if (dto.dataset) {
      try {
        const parsed = JSON.parse(dto.dataset);
        if (!Array.isArray(parsed)) {
          this.logger.warn('Invalid dataset JSON shape');
          return this.getEmptyReport('Invalid dataset format');
        }
        values = toFiniteNumberSeries(parsed);
      } catch {
  ...
  async detectAccountRisk(accountData: {
    ...
    const followerAnomalies = detectAnomalies(toFiniteNumberSeries(accountData.followers), 'high');
    const engagementAnomalies = detectAnomalies(toFiniteNumberSeries(accountData.engagement), 'medium');
    const frequencyAnomalies = detectAnomalies(
      toFiniteNumberSeries(accountData.publishFrequency),
      'medium',
    );
