...
export class TeamsService {
  ...
  async create(dto: CreateTeamDto, userId: string) {
    ...
    if (!organizationId) {
      throw new ForbiddenException('用户未加入任何组织，无法创建团队');
    }
  ...
  async inviteMember(dto: InviteMemberDto, inviterId: string) {
    ...
    if (!invitee) {
      throw new NotFoundException('该邮箱未注册，请先注册账号');
    }
  ...
  async updateMemberRole(
    ...
    if (!operator || !['OWNER', 'ADMIN'].includes(operator.role)) {
      throw new ForbiddenException('您没有修改成员角色的权限');
    }
  ...
  async removeMember(
    ...
    if (!operator || !['OWNER', 'ADMIN'].includes(operator.role)) {
      throw new ForbiddenException('您没有移除成员的权限');
    }
