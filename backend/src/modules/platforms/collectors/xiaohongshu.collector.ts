...
@Injectable()
export class XiaohongshuCollector implements IDataCollector {
  ...
  private async getClient(accountId: string): Promise<XiaohongshuClient> {
  ...
  async collectContentMetrics(
    ...
    try {
      ...
      const contents: ContentMetrics[] = noteList.data.notes.map((note) => ({
  ...
  async collectDailyMetrics(accountId: string): Promise<CollectorResult<DailyMetrics>> {
    try {
      ...
      await this.prisma.dailyStats.create({
        data: {
          accountId, date: new Date(), platform: 'XIAOHONGSHU' as any,
