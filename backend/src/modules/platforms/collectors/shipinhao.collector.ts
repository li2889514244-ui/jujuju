...
@Injectable()
export class ShipinhaoCollector implements IDataCollector {
  readonly platform = 'WECHAT_VIDEO';
  private readonly logger = new Logger(ShipinhaoCollector.name);
  ...
  private async getClient(accountId: string): Promise<ShipinhaoClient> {
    const account = await this.prisma.account.findUnique({ where: { id: accountId } });
    if (!account) throw new Error('账号不存在');

    const client = new ShipinhaoClient();
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          nickname: userInfo.nickname,
          followers: (userInfo as any).followers || 0,
          following: (userInfo as any).following || 0,
  ...
  async collectContentMetrics(
    ...
    try {
      ...
      const contents: ContentMetrics[] = videoList.data.video_list.map((video) => ({
  ...
  async collectDailyMetrics(accountId: string): Promise<CollectorResult<DailyMetrics>> {
    try {
      ...
      await this.prisma.dailyStats.create({
        data: {
          accountId, date: new Date(), platform: 'WECHAT_VIDEO' as any,
