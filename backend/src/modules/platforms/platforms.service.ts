...
export class PlatformsService {
  ...
  private isSuspiciousCollectedNickname(platform: unknown, value: unknown): boolean {
  ...
  private isSafeCollectedAvatar(value: unknown): boolean {
    if (typeof value !== 'string') return false
    const text = value.trim()
  ...
  private getAvatarExt(contentType: string, sourceUrl: string): string {
  ...
  private getAvatarContentType(filePath: string): string {
  ...
  private async cacheCollectedAvatar(accountId: string, avatarUrl: string): Promise<string | null> {
    if (avatarUrl.startsWith('/api/v1/platforms/avatar/')) return avatarUrl
    const safeAccountId = this.getSafeAvatarAccountId(accountId)
    if (!safeAccountId || !this.isSafeCollectedAvatar(avatarUrl)) return null
    ...
    try {
      ...
      if (buffer.length < 64 || buffer.length > 2 * 1024 * 1024) {
        throw new Error(`invalid avatar size ${buffer.length}`)
      }
      ...
      await Promise.all(
        existing
          ...
          .map((file) => fs.unlink(path.join(this.avatarCacheDir, file)).catch(() => undefined)),
      )
      const fileName = `${safeAccountId}-${hash}${ext}`
      const filePath = path.join(this.avatarCacheDir, fileName)
      await fs.writeFile(filePath, buffer)
      return `/api/v1/platforms/avatar/${safeAccountId}?v=${hash}`
    } catch (error: any) {
      this.logger.warn(`cache avatar failed for ${accountId}: ${error?.message || error}`)
      return null
    } finally {
  ...
  async getCachedAvatar(accountId: string): Promise<{ filePath: string; contentType: string } | null> {
    const safeAccountId = this.getSafeAvatarAccountId(accountId)
    if (!safeAccountId) return null
    const files: string[] = await fs.readdir(this.avatarCacheDir).catch(() => [])
    const fileName = files.find((file) => file.startsWith(`${safeAccountId}-`))
    if (!fileName) return null
    const filePath = path.join(this.avatarCacheDir, fileName)
    return { filePath, contentType: this.getAvatarContentType(filePath) }
  }
  ...
  async reportMetrics(dto: ReportMetricsDto) {
    ...
    const collectorLogs = Array.isArray(metrics?._collectorLogs)
      ? metrics._collectorLogs
          .map((line: unknown) => String(line).replace(/[\r\n]+/g, ' ').slice(0, 800))
          .filter((line: string) => line.trim().length > 0)
          .slice(-80)
      : []
    if (collectorLogs.length > 0) {
      this.logger.log(`reportMetrics: ${accountId} companion collector logs begin (${collectorLogs.length})`)
      for (const line of collectorLogs) {
        this.logger.log(`companionLog ${accountId}: ${line}`)
      }
      this.logger.log(`reportMetrics: ${accountId} companion collector logs end`)
    }
    ...
    try {
      ...
      const normalizeDailyMetric = (value: any) => {
        ...
        const hasMetricField = metricKeys.some((key) => Object.prototype.hasOwnProperty.call(value, key))
        const normalized = {
        ...
        return hasMetricField || Object.values(normalized).some((n) => n > 0) ? normalized : undefined
      }
      const periodKeys = ['day_total', 'week_total', 'month_total'] as const
      const describeVideoMetric = (value: any) => {
        const total = normalizeDailyMetric(value)
        return total
          ? `play=${total.play},like=${total.like},comment=${total.comment},share=${total.share},new_fans=${total.new_fans}`
          : 'missing'
      }
      const describePeriodKeys = (value: any) =>
        periodKeys.filter((key) => value?.[key]).join(',') || 'none'
      const videoPeriodRejectReasons: string[] = []
      const normalizeVideoPeriodMetrics = (value: any) => {
        ...
        if (
          ...
          videoPeriodRejectReasons.push('week_total_lt_day_total')
        ...
        if (
          ...
          videoPeriodRejectReasons.push(
            normalized.week_total ? 'month_total_lt_week_total' : 'month_total_lt_day_total',
          )
        ...
        if (
          ...
          videoPeriodRejectReasons.push('day_week_month_identical')
          delete normalized.week_total
          delete normalized.month_total
        }
      ...
      const normalizeFollowerPeriodMetrics = (value: any) => {
        if (!value || typeof value !== 'object') return undefined
        const normalizeFollowerMetric = (metric: any) => {
          const normalized = {
            new_fans: pickNumber(metric?.new_fans) ?? 0,
            unfollows: pickNumber(metric?.unfollows) ?? 0,
            net_fans: pickNumber(metric?.net_fans) ?? 0,
          }
          return Object.values(normalized).some((n) => n !== 0) ? normalized : undefined
        }
        const normalized: Record<string, any> = {}
        for (const key of ['day_total', 'week_total', 'month_total']) {
          const total = normalizeFollowerMetric(value[key])
          if (total) normalized[key] = total
        }
        if (typeof value.source === 'string') normalized.source = value.source
        if (typeof value.collectedAt === 'string') normalized.collectedAt = value.collectedAt
      ...
      // ── 重复数据检测 ──
      // 如果本次上报的所有字段与前次记录完全相同，仍写入本次采集日期，
      // 但把增量字段归零：既保留“确实采集过”的新鲜度，又避免复制旧增量。
      if (previousStats) {
        const allSame = [
          'followers',
          'views',
        ...
        if (allSame && Object.values(statData).some((v) => v !== undefined && v !== 0)) {
          this.logger.debug(
            `reportMetrics: duplicate totals for ${accountId} on ${targetDate.toISOString()}; writing freshness row with zero increments`,
          )
          statData.followersIncrement = 0
          statData.viewsIncrement = 0
          statData.likesIncrement = 0
          statData.commentsIncrement = 0
          statData.sharesIncrement = 0
        }
      }
      ...
      if (
        ...
        nickname.length >= 2 &&
        !this.isSuspiciousCollectedNickname(platform, nickname)
      )
        accountUpdates.nickname = nickname
      else if (this.isSuspiciousCollectedNickname(platform, nickname))
      ...
      const rawVideoPeriodMetrics = metrics?._periodMetrics?.videoData
      const videoPeriodMetrics = normalizeVideoPeriodMetrics(rawVideoPeriodMetrics)
      const followerPeriodMetrics = normalizeFollowerPeriodMetrics(metrics?._periodMetrics?.followerData)
      if (platform === 'WECHAT_VIDEO' && rawVideoPeriodMetrics) {
        this.logger.log(
          `reportMetrics: ${accountId} received video periodMetrics source=${rawVideoPeriodMetrics.source || 'unknown'} ` +
            `keys=${describePeriodKeys(rawVideoPeriodMetrics)} day=${describeVideoMetric(rawVideoPeriodMetrics.day_total)} ` +
            `week=${describeVideoMetric(rawVideoPeriodMetrics.week_total)} month=${describeVideoMetric(rawVideoPeriodMetrics.month_total)}`,
        )
        if (!videoPeriodMetrics?.month_total) {
          const rawMonth = normalizeDailyMetric(rawVideoPeriodMetrics.month_total)
          this.logger.warn(
            `reportMetrics: ${accountId} video month_total not saved rawHadMonth=${Boolean(rawMonth)} ` +
              `reasons=${videoPeriodRejectReasons.join(',') || 'missing_or_all_zero'} rawMonth=${describeVideoMetric(rawVideoPeriodMetrics.month_total)}`,
          )
        }
      }
      if (videoPeriodMetrics || followerPeriodMetrics) {
        ...
        accountUpdates.metadata = JSON.stringify({
          ...
          periodMetrics: {
            ...periodMetrics,
            ...(videoPeriodMetrics ? { videoData: videoPeriodMetrics } : {}),
            ...(followerPeriodMetrics ? { followerData: followerPeriodMetrics } : {}),
          },
          companionPeriodMetricsUpdatedAt: new Date().toISOString(),
        })
      }
      {
        const metadata = parseJsonObject(accountUpdates.metadata || account.metadata)
        accountUpdates.metadata = JSON.stringify({
          ...metadata,
          companionSession: {
            status: 'online',
  ...
  async reportSessionStatus(dto: ReportSessionStatusDto) {
    ...
    const account = await this.prisma.account.findUnique({ where: { id: accountId } })
    if (!account) {
      this.logger.warn(`reportSessionStatus: account ${accountId} not found`)
      return { success: false, error: 'Account not found' }
    }
    ...
    await this.prisma.account.update({
      ...
      data: {
        ...
        ...(online ? { lastActiveAt: checkedAt } : {}),
        metadata: JSON.stringify({
          ...metadata,
          companionSession: {
            status,
            source: String(dto.source || 'companion').slice(0, 80),
            reason: String(dto.reason || '').slice(0, 240),
    ...
    this.logger.log(
      `reportSessionStatus: ${accountId} status=${status} source=${dto.source || 'companion'}`,
    )
    return { success: true, online, status }
  }
  ...
  async reportPostStats(dto: ReportPostStatsDto) {
    ...
    for (const p of posts) {
      try {
        const externalId = String(
          p.id || p.content_id || p.contentId || (p as any).contentID || (p as any).aweme_id || (p as any).awemeId || '',
        ).trim()
        const title = p.title || externalId || ''
        if (!title && !externalId) {
          continue
        }
        const coverUrl = p.coverUrl || (p as any).cover_url || (p as any).cover || undefined
        const publishAt = this.parseReportedPostDate(
          p.publishedAt || (p as any).publish_time || (p as any).published_at || p.date,
        )
        const metadata = externalId
          ? JSON.stringify({ externalId, source: 'desktop-companion' })
          : undefined

        // Prefer the platform's own post id. Title is only a fallback because
        // repeated/empty titles are common in imported video lists.
        let existing: any = null
        if (externalId) {
          existing = await this.prisma.post.findFirst({
            where: { accountId, metadata: { contains: externalId } },
          })
        }
        if (!existing && !externalId && title) {
          existing = await this.prisma.post.findFirst({
        ...
        if (existing) {
          ...
          if (coverUrl) postUpdates.coverUrl = coverUrl
          if (publishAt && !existing.publishAt) postUpdates.publishAt = publishAt
          if (metadata && !String(existing.metadata || '').includes(externalId)) {
            postUpdates.metadata = metadata
          }
    ...
    const allPostTotals = await this.prisma.postStats.aggregate({
    ...
    const postTotals = {
      views: allPostTotals._sum.views ?? 0,
      likes: allPostTotals._sum.likes ?? 0,
      comments: allPostTotals._sum.comments ?? 0,
      shares: allPostTotals._sum.shares ?? 0,
    }
    const hasPostTotals = Object.values(postTotals).some((value) => value > 0)
    if (hasPostTotals) {
      const targetDate = this.getBeijingDayStart(0)
      await this.prisma.dailyStats.upsert({
        where: { accountId_date: { accountId, date: targetDate } },
        update: {
