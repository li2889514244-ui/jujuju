...
export class OAuthService {
  ...
  async refreshAccountToken(accountId: string): Promise<boolean> {
    ...
    const metadata = this.parseAccountMetadata(accountId, account.metadata);
    if (!metadata) {
      return false;
    }
  ...
  private parseAccountMetadata(accountId: string, raw?: string | null): Record<string, any> | null {
    if (!raw) return {};
    try {
      const parsed = JSON.parse(raw);
      return parsed && typeof parsed === 'object' ? parsed : {};
    } catch (error: any) {
      this.logger.warn(`账号 ${accountId} OAuth metadata 格式异常，跳过Token刷新: ${error.message}`);
      return null;
    }
  }
