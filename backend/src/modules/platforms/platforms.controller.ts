...
export class PlatformsController {
  ...
  async getCachedAvatar(@Param('accountId') accountId: string, @Res() res: Response) {
    const avatar = await this.platformsService.getCachedAvatar(accountId)
    if (!avatar) {
  ...
  @Post('report-session-status')
  @ApiOperation({ summary: '桌面伴侣上报账号登录态实测结果' })
  @HttpCode(HttpStatus.OK)
