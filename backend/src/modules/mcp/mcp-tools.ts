/**
 * MCP Tool 定义文件 — 数据查询工具集
 *
 * 每个 Tool 包含 name / description / inputSchema，
 * 遵循 Model Context Protocol 的 Tool 注册规范。
 * 所有查询均为只读操作，不开放任何写权限。
 */
...
const queryAccountDataTool: McpTool = {
  name: 'query_account_data',
  description:
    '查询指定账号在某个时间段内的详细数据，包括粉丝、播放量、点赞、评论、分享、GMV、订单等。支持按平台过滤。',
  inputSchema: {
    type: 'object',
    properties: {
      accountName: {
...
const getTopRankingsTool: McpTool = {
  name: 'get_top_rankings',
  description:
    '获取指定指标的排行榜，按降序排列。支持 GMV、粉丝数、点赞数、播放量等维度，可按周/月/总计筛选。',
  inputSchema: {
    type: 'object',
    properties: {
      metric: {
...
const compareAccountsTool: McpTool = {
  name: 'compare_accounts',
  description:
    '对比多个账号在同一指标上的表现，返回每个账号的数据及差异分析。适用于竞品分析或团队内部账号对比。',
  inputSchema: {
    type: 'object',
    properties: {
      accountNames: {
...
const generateReportTool: McpTool = {
  ...
  description:
    '生成周报或月报，汇总指定账号（或全平台）的数据，包括粉丝增长、播放量、互动率、GMV 等关键指标。',
  inputSchema: {
    type: 'object',
    properties: {
      accountName: {
...
const exportDataTool: McpTool = {
  name: 'export_data',
  description:
    '导出指定账号在时间段内的数据为 CSV 格式。可选指定导出列，默认导出全部字段。',
  inputSchema: {
    type: 'object',
    properties: {
      accountName: {
      ...
      columns: {
        ...
        items: { type: 'string' },
        description:
          '导出列名，可选：date/platform/nickname/followers/views/likes/comments/shares/revenue/gmv/orders/commission/buyerCount。不传则全部导出。',
      },
    },
