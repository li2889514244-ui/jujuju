...
export class WechatStoreService implements OnModuleInit {
  ...
  async createStore(name: string, appId: string, appSecret: string) {
    return this.prisma.wechatStore.create({
  ...
  private async fetchToken(appId: string, appSecret: string, label: string): Promise<string> {
    ...
    if (data.errcode) {
      this.logger.error(`[${label}] token fail: ${data.errcode}`)
  ...
  async getOrderList(
    ...
    return this.request(storeId, '/channels/ec/order/list/get', {
      ...
      create_time_range: { start_time: now - 7 * 86400, end_time: now },
  ...
  async getAftersaleListAggregated(
    storeId: string,
    params: { begin_create_time?: number; end_create_time?: number; next_key?: string } = {},
  ) {
    const res: any = await this.getAftersaleList(storeId, params)
    ...
    const list = details
      ...
      .map((d: any) => {
        ...
        return {
          id: a.after_sale_order_id,
          type: a.type,
          status: a.status,
          amount: a.refund_info?.amount || 0,
          reason: a.reason_text || '',
          product: productMap[String(pid)] || `商品${pid}`,
          complete_time: a.complete_time || 0,
  ...
  async getOrderListAggregated(
    ...
    const order_list = details
      .filter((d: any) => d.errcode === 0)
      .map((d: any) => {
        const o = d.order || {}
        const info = o.order_detail?.product_infos?.[0] || {}
        ...
        return {
          ...
          sku_id: info.sku_id || '',
          status: o.status || 0,
          pay_amount: o.order_detail?.price_info?.order_price || 0,
          create_time: o.create_time || 0,
          settle_time: o.settle_time || 0,
          product_title: info.title || '',
          product_img: info.thumb_img || '',
          ship_time: dlv?.ship_done_time || 0,
          delivery_list: dlv?.delivery_product_info || [],
          commission: 0,
          commission_rate: 0,
        }
      })
  ...
  async getProductListAggregated(
    storeId: string,
    params: { page_size?: number; next_key?: string },
  ) {
    ...
    const products = details
      ...
      .map((d: any) => {
        ...
        return {
          ...
          title: p.title || '',
          img_url: p.head_imgs?.[0] || '',
          selling_price: sku.sale_price || 0,
          sales: p.total_sold_num || 0,
