...
export class HealthService {
  ...
  private checkMemory(): {
    ...
    const maxPercentage = Number(process.env.HEALTH_MEMORY_MAX_PERCENTAGE || 90)
    const maxRssMB = Number(process.env.HEALTH_MEMORY_MAX_RSS_MB || 1536)

    return {
      status: percentage > maxPercentage && usedMB > maxRssMB ? 'error' : 'ok',
      usedMB,
      totalMB,
      percentage,
    }
