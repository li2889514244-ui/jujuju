...
export class FeishuOpenApiClient {
  ...
  buildCard(params: {
    ...
    if (params.type !== 'REPORT' && params.type !== 'REFUND_ALERT') {
      elements.push({ tag: 'hr' })
      const noteElements: Record<string, any>[] = [
        { tag: 'plain_text', content: `类型: ${params.type}` },
      ]

      if (params.userId) {
        noteElements.push({ tag: 'plain_text', content: `用户: ${params.userId}` })
      }
      ...
      if (params.metadata) {
        elements.push({
          ...
          elements: [
            {
              tag: 'plain_text',
              content: `附加信息: ${this.truncate(JSON.stringify(params.metadata), 500)}`,
            },
          ],
        })
      }
