...
describe('SystemHealthService', () => {
  ...
  describe('伴侣故障同步去重（串行化防竞态）', () => {
    it('并发同步只创建一次故障，第二次走更新路径', async () => {
      ...
      mockPrismaService.systemIncident.findFirst.mockImplementation(async () =>
  ...
  describe('getOverview 计数', () => {
    function setupOverview(
      ...
      mockPrismaService.$queryRaw.mockResolvedValue(
        overrides.dbOk === false ? Promise.reject(new Error('db down')) : [],
      )
    }
    ...
    it('尚未执行采集不计为失败，且异常设备数与故障条数分开统计', async () => {
      ...
      mockPrismaService.companionIncident.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([
    ...
    it('已恢复的前端错误只保留为24小时趋势，不继续把卡片标成降级', async () => {
      ...
      mockPrismaService.systemIncident.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([])
