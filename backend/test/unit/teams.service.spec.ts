...
describe('TeamsService', () => {
  ...
  describe('create', () => {
    ...
    it('用户无组织时应抛出 ForbiddenException', async () => {
      ...
      await expect(service.create(createDto, 'user-005')).rejects.toThrow(
        '用户未关联任何组织，无法创建团队',
      );
    });
  ...
  describe('inviteMember', () => {
    ...
    it('管理员应能邀请新成员', async () => {
      mockPrismaService.user.findUnique
        .mockResolvedValueOnce({ ...mockUsers.regular, email: inviteDto.email, organizationId: null }) // 查找被邀请人
