...
describe('AccountsService', () => {
  ...
  describe('create', () => {
    ...
    it('平台账号已存在时应通过 upsert 更新', async () => {
  ...
  describe('update', () => {
    ...
    it('共享模式下非所有者也可以更新', async () => {
      ...
      const result = await service.update('acc-001', updateDto, 'user-other');
  ...
  describe('remove', () => {
    ...
    it('共享模式下非所有者也可以删除', async () => {
      mockPrismaService.account.findUnique.mockResolvedValue(mockAccounts.douyin);
      mockPrismaService.user.findUnique.mockResolvedValue(mockUsers.regular);

      const result = await service.remove('acc-001', 'user-other');
  ...
  describe('getCookies', () => {
    ...
    it('Cookie 密文损坏时应返回 null，避免接口 500', async () => {
      mockPrismaService.account.findUnique.mockResolvedValue({
        ...mockAccounts.douyin,
        cookies: 'not:a:valid:encrypted-cookie',
      });
    ...
    it('共享模式下非所有者也可以获取 Cookie', async () => {
      mockPrismaService.account.findUnique.mockResolvedValue({
        ...mockAccounts.douyin,
        cookies: (service as any).encryptCookie('shared_cookie'),
      });
      mockPrismaService.user.findUnique.mockResolvedValue(mockUsers.regular);

      const result = await service.getCookies('acc-001', 'user-other');

      expect(result.cookies).toBe('shared_cookie');
    });
  });
  ...
  describe('Cookie 加密解密', () => {
    ...
    it('加密格式应为 iv:authTag:encrypted（hex 格式）', async () => {
      mockPrismaService.account.findUnique.mockResolvedValue(null);
      mockPrismaService.account.create.mockResolvedValue(mockAccounts.douyin);
