...
import { PostStatus } from '../../src/common/prisma-enums';
...
describe('ContentService', () => {
  ...
  describe('create', () => {
    ...
    it('关联账号不存在时应抛出 NotFoundException', async () => {
      ...
      await expect(service.create(createDto, 'user-001')).rejects.toThrow('账号不存在');
    });
    ...
    it('标签为空时应使用空数组', async () => {
      ...
      const createCall = mockPrismaService.post.create.mock.calls[0][0];
      expect(createCall.data.tags).toBe('[]');
    });
  });
  ...
  describe('getScheduledPosts', () => {
    ...
    it('应包含发布所需的账号标识信息', async () => {
      mockPrismaService.post.findMany.mockResolvedValue([mockPosts.scheduled]);

      await service.getScheduledPosts();

      const findCall = mockPrismaService.post.findMany.mock.calls[0][0];
      expect(findCall.include.account.select).toHaveProperty('id');
      expect(findCall.include.account.select).toHaveProperty('platform');
      expect(findCall.include.account.select).toHaveProperty('userId');
    });
